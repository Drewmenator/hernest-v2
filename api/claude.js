...
export default async function handler(req, res) {
  ...
  const effectiveMaxTokens = ["morning_briefing","trip_planner","meal_plan","style_stylist"].includes(feature) ? Math.max(max_tokens, 2000) : max_tokens;
  ...
  if (max_tokens > 6000) return res.status(400).json({ error: "max_tokens too large" });
