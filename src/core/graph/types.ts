...
// Proactive alert/reminder (budget threshold, trip prep, circle nudge) — already
// generated by connectivity.ts; surfaced to Cleo so she can remind, like a PA.
export interface AlertItem {
  type: string;                     // "budget" | "trip" | "circle" | ...
  message: string;
...
export interface HouseholdContextGraph {
  ...
  alerts: AlertItem[];              // proactive reminders (budget/trip/circle)
  goals: Goal[];
...
export interface CleoContextPack {
  ...
  alerts: string[];           // proactive reminders, most severe first
  activeGoals: Array<{
    title: string;
    category: string;
    riskStatus: string;
