...
export interface Category {
  id: string; label: string; budget: number; spent: number; color: string; icon: string;
}
export interface Expense {
  id: string; amount: number; category: string; merchant: string; note: string; date: string; method: "manual" | "receipt" | "csv";
}
export interface Income {
  id: string; label: string; amount: number; frequency: "monthly" | "biweekly" | "weekly" | "annual"; type: "salary" | "freelance" | "rental" | "other";
}
export interface Debt {
  id: string; label: string; balance: number; apr: number; minimumPayment: number; monthlyPayment: number;
  type: "credit_card" | "student_loan" | "car_loan" | "mortgage" | "personal" | "other"; payoffDate?: string;
}
export interface FinancialGoal {
  id: string; name: string;
  type: "emergency_fund" | "vacation" | "school_fees" | "medical" | "home" | "car" | "debt_payoff" | "family_event" | "other";
  targetAmount: number; currentAmount: number; targetDate?: string; monthlyContribution: number;
  riskStatus: "on_track" | "at_risk" | "off_track"; aiRecommendation?: string; linkedDebtId?: string;
}
export interface Scenario {
  id: string; question: string; result?: ScenarioResult; createdAt: string;
}
export interface ScenarioResult {
  financialImpact: string; tradeoffs: string[]; riskLevel: "low" | "medium" | "high";
  recommendedAction: string; confidenceLevel: number;
}
export interface AIInsight {
  id: string; observation: string; whyItMatters: string; options: string[];
  recommendation: string; confidenceLevel: number;
  category: "spending" | "savings" | "debt" | "cashflow" | "stress"; createdAt: string;
}
...
export interface MonthlyBudgetSummary {
  totalIncome: number; fixedExpenses: number; variableExpenses: number;
  cashRemaining: number; savingsRate: number; totalDebt: number; debtToIncomeRatio: number;
}
...
export function computePayoffDate(debt: Debt): string {
  ...
  const months = Math.ceil(-Math.log(1 - (r * debt.balance) / debt.monthlyPayment) / Math.log(1 + r));
  if (!isFinite(months) || months < 0) return "Payment too low";
  const d = new Date(); d.setMonth(d.getMonth() + months);
  return d.toLocaleDateString("en-US", { month: "short", year: "numeric" });
}

export function computeTotalInterest(debt: Debt): number {
  if (debt.monthlyPayment <= 0 || debt.balance <= 0) return 0;
  const r = debt.apr / 100 / 12;
  if (r === 0) return 0;
  const months = Math.ceil(-Math.log(1 - (r * debt.balance) / debt.monthlyPayment) / Math.log(1 + r));
  if (!isFinite(months) || months < 0) return 0;
  return Math.round((debt.monthlyPayment * months) - debt.balance);
}

export function gradeScore(score: number): "A" | "B" | "C" | "D" | "F" {
  if (score >= 85) return "A"; if (score >= 70) return "B";
  if (score >= 55) return "C"; if (score >= 40) return "D"; return "F";
}
